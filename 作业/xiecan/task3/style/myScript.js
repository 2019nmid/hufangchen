(function (window, document) {
    var SliderBar = function (targetDom, options) {
        ...
        // 获取dom
        targetDom = document.getElementById(targetDom);
        var dataList = options.dataList;
        if (dataList.length > 0) {
            this.targetDom.innerHTML = html;
            var slideBtn = document.getElementById("slideBtn");                 // 拖拽按钮
            var refreshBtn = document.getElementById("refreshBtn");             // 换图按钮
            var slideHint = document.getElementById("slideHint");               // 提示名称
            var slideImg = document.getElementById("slideImg");                 // 图片
            var cutBlock = document.getElementById("cutBlock");                 // 裁剪区域
            var slideBlock = document.getElementById("slideBlock");             // 裁剪的图片
            var slideIcon = document.getElementById("slideIcon");               // 正确、失败的图标
            var slideType = document.getElementById("slideType");               // 正确、失败
            var slideContent = document.getElementById("slideContent");         // 正确、失败的正文
            var slideHintInfo = document.getElementById("slideHintInfo");       // 弹出
            var resultX = 0;
            var startX = 0;
            var timer = 0;
            var startTamp = 0;
